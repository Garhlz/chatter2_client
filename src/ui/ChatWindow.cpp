...
ChatWindow::ChatWindow(ChatClient *client, const QString &nickname, QWidget *parent)
    : QMainWindow(parent), chatClient(client), nickname(nickname), isInitialized(false) {
...
void ChatWindow::setupUi() {
    try {
        ...
        if (!centralWidget) throw std::runtime_error("Failed to create centralWidget");
        ...
        if (!chatTabs) throw std::runtime_error("Failed to create chatTabs");
        mainLayout->addWidget(chatTabs);

        // 公共聊天页
        qDebug() << "ChatWindow: Setting up public chat tab";
        publicChatTab = new QWidget();
        if (!publicChatTab) throw std::runtime_error("Failed to create publicChatTab");
        QVBoxLayout *publicLayout = new QVBoxLayout(publicChatTab);
        if (!publicLayout) throw std::runtime_error("Failed to create publicLayout");

        publicChatDisplay = new QScrollArea();
        if (!publicChatDisplay) throw std::runtime_error("Failed to create publicChatDisplay");
        publicChatContainer = new QWidget();
        if (!publicChatContainer) throw std::runtime_error("Failed to create publicChatContainer");
        publicChatLayout = new QVBoxLayout(publicChatContainer);
        if (!publicChatLayout) throw std::runtime_error("Failed to create publicChatLayout");
        publicChatLayout->addStretch();
        publicChatDisplay->setWidget(publicChatContainer);
        publicChatDisplay->setWidgetResizable(true);
        publicChatDisplay->setStyleSheet("border: 1px solid #ccc;");

        qDebug() << "ChatWindow: Creating public input layout";
        QHBoxLayout *publicInputLayout = new QHBoxLayout();
        if (!publicInputLayout) throw std::runtime_error("Failed to create publicInputLayout");
        publicMessageInput = new QLineEdit();
        if (!publicMessageInput) throw std::runtime_error("Failed to create publicMessageInput");
        publicSendButton = new QPushButton("发送");
        if (!publicSendButton) throw std::runtime_error("Failed to create publicSendButton");
        ...
        if (!privateChatTab) throw std::runtime_error("Failed to create privateChatTab");
        QHBoxLayout *privateLayout = new QHBoxLayout(privateChatTab);
        if (!privateLayout) throw std::runtime_error("Failed to create privateLayout");
        QVBoxLayout *privateChatLayout = new QVBoxLayout();
        if (!privateChatLayout) throw std::runtime_error("Failed to create privateChatLayout");

        qDebug() << "ChatWindow: Creating private chat display";
        privateChatDisplay = new QScrollArea();
        if (!privateChatDisplay) throw std::runtime_error("Failed to create privateChatDisplay");
        privateChatContainer = new QWidget();
        if (!privateChatContainer) throw std::runtime_error("Failed to create privateChatContainer");
        QVBoxLayout *privateMessagesLayout = new QVBoxLayout(privateChatContainer);
        if (!privateMessagesLayout) throw std::runtime_error("Failed to create privateMessagesLayout");
        privateMessagesLayout->addStretch();
        qDebug() << "ChatWindow: Setting privateChatContainer to privateChatDisplay";
        privateChatDisplay->setWidget(privateChatContainer);
        privateChatDisplay->setWidgetResizable(true);
        privateChatDisplay->setStyleSheet("border: 1px solid #ccc;");

        qDebug() << "ChatWindow: Creating private input layout";
        QHBoxLayout *privateInputLayout = new QHBoxLayout();
        if (!privateInputLayout) throw std::runtime_error("Failed to create privateInputLayout");
        qDebug() << "ChatWindow: Creating private message input";
        privateMessageInput = new QLineEdit();
        if (!privateMessageInput) throw std::runtime_error("Failed to create privateMessageInput");
        qDebug() << "ChatWindow: Creating private send button";
        privateSendButton = new QPushButton("发送");
        if (!privateSendButton) throw std::runtime_error("Failed to create privateSendButton");
        qDebug() << "ChatWindow: Creating send file button";
        sendFileButton = new QPushButton("发送文件");
        if (!sendFileButton) throw std::runtime_error("Failed to create sendFileButton");
        ...
        if (!usersLayout) throw std::runtime_error("Failed to create usersLayout");
        QLabel *usersLabel = new QLabel("在线用户");
        if (!usersLabel) throw std::runtime_error("Failed to create usersLabel");
        onlineUsersList = new QListWidget();
        if (!onlineUsersList) throw std::runtime_error("Failed to create onlineUsersList");
        ...
        if (!groupChatTab) throw std::runtime_error("Failed to create groupChatTab");
        QVBoxLayout *groupLayout = new QVBoxLayout(groupChatTab);
        if (!groupLayout) throw std::runtime_error("Failed to create groupLayout");

        groupChatDisplay = new QScrollArea();
        if (!groupChatDisplay) throw std::runtime_error("Failed to create groupChatDisplay");
        groupChatContainer = new QWidget();
        if (!groupChatContainer) throw std::runtime_error("Failed to create groupChatContainer");
        groupChatLayout = new QVBoxLayout(groupChatContainer);
        if (!groupChatLayout) throw std::runtime_error("Failed to create groupChatLayout");
        groupChatLayout->addStretch();
        groupChatDisplay->setWidget(groupChatContainer);
        groupChatDisplay->setWidgetResizable(true);
        groupChatDisplay->setStyleSheet("border: 1px solid #ccc;");

        qDebug() << "ChatWindow: Creating group input layout";
        QHBoxLayout *groupInputLayout = new QHBoxLayout();
        if (!groupInputLayout) throw std::runtime_error("Failed to create groupInputLayout");
        groupCombo = new QComboBox();
        if (!groupCombo) throw std::runtime_error("Failed to create groupCombo");
        groupCombo->setPlaceholderText("选择群组");
        groupMessageInput = new QLineEdit();
        if (!groupMessageInput) throw std::runtime_error("Failed to create groupMessageInput");
        groupMessageInput->setPlaceholderText("消息内容");
        groupSendButton = new QPushButton("发送");
        if (!groupSendButton) throw std::runtime_error("Failed to create groupSendButton");
        ...
        if (!statusBar) throw std::runtime_error("Failed to create statusBar");
        statusLabel = new QLabel("已连接");
        if (!statusLabel) throw std::runtime_error("Failed to create statusLabel");
        onlineCountLabel = new QLabel("在线人数: 0");
        if (!onlineCountLabel) throw std::runtime_error("Failed to create onlineCountLabel");
        QPushButton *logoutButton = new QPushButton("登出");
        if (!logoutButton) throw std::runtime_error("Failed to create logoutButton");
        ...
        setStyleSheet(
            ...
            "QPushButton { padding: 8px; background-color: #4CAF50; color: white; border: none; }"
...
void ChatWindow::connectSignals() {
    try {
        ...
        connect(chatClient, &ChatClient::historyMessagesReceived, this, &ChatWindow::handleHistoryMessagesReceived);
...
void ChatWindow::sendGroupMessage() {
    ...
    QString timestamp = QDateTime::currentDateTime().toString("hh:mm:ss");
    appendMessageBubble(groupChatContainer, nickname, QString("[%1] %2").arg(groupName).arg(content), timestamp);
...
void ChatWindow::sendFile() {
    ...
    QString timestamp = QDateTime::currentDateTime().toString("hh:mm:ss");
    appendMessageBubble(privateChatContainer, nickname, "[发送文件] " + QFileInfo(filePath).fileName(), timestamp);
    file.close();
}
    ...
    QFuture<void> future = QtConcurrent::run([=]() {
        ...
        if (file.open(QIODevice::WriteOnly)) {
            ...
            QMetaObject::invokeMethod(this, [=]() {
                QString timestamp = QDateTime::currentDateTime().toString("hh:mm:ss");
                appendMessageBubble(privateChatContainer, sender, "[接收文件] " + QFileInfo(saveFilePath).fileName(), timestamp);
                if (messageId > 0) displayedMessages.insert(messageId);
            });
        }
    });
...
void ChatWindow::handleOnlineUsersUpdated(const QJsonArray &users, int count) {
    ...
    try {
        ...
        qDebug() << "ChatWindow: Qt exception in handleOnlineUsersUpdated:" << e.what();
        QMessageBox::warning(this, "错误", QString("更新在线用户失败: %1").arg(e.what()));
    } catch (const std::exception &e) {
        qDebug() << "ChatWindow: Exception in handleOnlineUsersUpdated:" << e.what();
        QMessageBox::warning(this, "错误", QString("更新在线用户失败: %1").arg(e.what()));
    } catch (...) {
...
void ChatWindow::handleGroupListReceived(const QJsonArray &groups) {
    ...
    try {
        ...
        qDebug() << "ChatWindow: Qt exception in handleGroupListReceived:" << e.what();
        QMessageBox::warning(this, "错误", QString("更新群组列表失败: %1").arg(e.what()));
    } catch (const std::exception &e) {
...
void ChatWindow::handleHistoryMessagesReceived(const QJsonArray &messages) {
    ...
    try {
        for (const QJsonValue &msg : messages) {
            ...
            if (!message.contains("type") || !message.contains("nickname") || !message.contains("content")) {
                continue;
            }
            ...
            if (type == "CHAT") {
                ...
                displayContent = QString("[%1] %2").arg(message["groupName"].toString()).arg(content);
        ...
        qDebug() << "ChatWindow: Qt exception in handleHistoryMessagesReceived:" << e.what();
        QMessageBox::warning(this, "错误", QString("加载历史消息失败: %1").arg(e.what()));
    } catch (const std::exception &e) {
        qDebug() << "ChatWindow: Exception in handleHistoryMessagesReceived:" << e.what();
        QMessageBox::warning(this, "错误", QString("加载历史消息失败: %1").arg(e.what()));
    } catch (...) {
        qDebug() << "ChatWindow: Unknown exception in handleHistoryMessagesReceived";
...
void ChatWindow::handleUserSelected() {
    try {
        ...
        QMessageBox::warning(this, "错误", QString("选择用户失败: %1").arg(e.what()));
    } catch (const std::exception &e) {
...
void ChatWindow::appendMessageBubble(QWidget *container, const QString &sender, const QString &content,
                                    const QString &timestamp, const QString &avatar) {
    try {
        ...
        MessageBubble *bubble = new MessageBubble(avatar, sender, content, timestamp, sender == nickname, container);
        layout->addWidget(bubble);
        layout->addStretch();

        QScrollArea *scrollArea = qobject_cast<QScrollArea*>(container->parentWidget()->parentWidget());
        if (scrollArea) {
        ...
        QMessageBox::warning(this, "错误", QString("创建消息气泡失败: %1").arg(e.what()));
    } catch (const std::exception &e) {
...
void ChatWindow::updateOnlineUsersList(const QJsonArray &users) {
    try {
        ...
        qDebug() << "ChatWindow: Qt exception in updateOnlineUsersList:" << e.what();
        throw;
    } catch (const std::exception &e) {
