...
PublicChatTab::PublicChatTab(ChatClient *client, const QString &nickname,
                             QWidget *parent)
...
void PublicChatTab::setupUi() {
  ...
  publicChatDisplay->setSizePolicy(QSizePolicy::Expanding,
                                   QSizePolicy::Expanding);
  publicChatDisplay->setHorizontalScrollBarPolicy(
...
void PublicChatTab::appendMessage(const QString &sender, const QString &content,
  ...
  QVBoxLayout *layout =
      qobject_cast<QVBoxLayout *>(publicChatContainer->layout());
  if (!layout) {
  ...
  MessageBubble *bubble = new MessageBubble(
      "", sender, content, timestamp, sender == nickname, publicChatContainer);

  // 不设置 SizePolicy，让气泡自然适应内容
  layout->addWidget(bubble, 0,
                    sender == nickname ? Qt::AlignRight : Qt::AlignLeft);

  // 只在末尾添加一个伸缩项，而不是每次都添加
  if (layout->count() == 0 ||
      !layout->itemAt(layout->count() - 1)->spacerItem()) {
    layout->addStretch();
  }
