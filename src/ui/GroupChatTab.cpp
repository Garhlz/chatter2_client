...
GroupChatTab::GroupChatTab(ChatClient *client, const QString &nickname,
                           QWidget *parent)
...
void GroupChatTab::sendGroupMessage() {
  ...
  appendMessage(nickname, QString("[%1] %2").arg(groupName).arg(content),
                timestamp);
...
void GroupChatTab::appendMessage(const QString &sender, const QString &content,
  ...
  QVBoxLayout *layout =
      qobject_cast<QVBoxLayout *>(groupChatContainer->layout());
  if (!layout) {
  ...
  MessageBubble *bubble = new MessageBubble(
      "", sender, content, timestamp, sender == nickname, groupChatContainer);
